...
`-Suite savebench` (also in `all`) evaluates the persistent real-save corpus via `tools/navigation/savebench.py --test`. Each case must load the exact ZIP in a fresh headless process, compare observed facts, and report zero geometry setup calls. The default matrix is all 45 native execution cases. Pure-model cases are not native arrivals. Preserve source ZIPs and bind derived grids/meshes/state tables to ZIP SHA-256, facts hash, state key and compiler settings; final acceptance and performance must run on the real map. Current algorithms may recompile derived action/controller state without modifying source geometry. Read `docs/save-backed-testbench.md` before changing this boundary. Source maps live outside the checkout under the sibling `factorio-scv-testbench/corpora`; `--build` creates a new version and never overwrites an existing corpus. Explicit user-requested authoritative test maps are separate from the legacy interactive developer-save refresh rule below.

`-Suite stepped` (also in `all`) runs saved-map external-solver correctness with native `tick_paused`/`ticks_to_run`. It must retain an independent host wall-time guard, shared result admission and true native arrival. Keep ordinary realtime live tests too. Report paused solver/capture wall time separately; stepping cannot qualify an algorithm as realtime. LuaProfiler timings are diagnostic host data, never synchronized decision inputs. Manual GUI launchers may be generated but never executed automatically.

`-Suite compare` (also in `all`) loads the eleven persistent static source saves with production-v1, grid A*, grid Dijkstra and source polygons. Its 44-row denominator includes every failure and no-path case. Read `docs/saved-solver-comparison.md`; keep the static corpus pointer separate from the 45 domain maps. Current code may change derived topology, never the saved source geometry/actor/task. A* and Dijkstra must agree on the same captured graph; native execution and wall/native timing remain separate from solver success. Polygon workers use the pinned Python 3.12 dependencies, supplied by `-TopologyPython` or the sibling `factorio-scv-testbench/runtime-static` environment. The default suite must fail rather than silently skip a missing backend.
