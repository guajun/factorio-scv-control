...
## 2026-09-16 - One interactive save without replacing the source-map oracle

**Question:** Can a human select all 56 current scenes in one save, compare fixed reference routes and freely right-click, while automation loads that exact published lab ZIP?

**Boundary:** `codex/unified-test-lab`, based on comparison commit `3917ab8`. A marker-gated hub orchestrates shared comparison/savebench adapters. Its 11 static scenes expose production planning plus three explicitly recorded external candidates; 10 gate, 5 dynamic and 30 belt scenes reuse native domain execution. Free play suspends the experimental adapter and attaches the character to normal production input. Rebuilding the selected convenience scene is allowed here; the existing 56 authoritative source ZIPs and their no-rebuild evaluation remain unchanged.

**First failed hypothesis:** Calling the same geometry builder does not necessarily restore identical native facts. The first package `v1-20260916-104958` selected all 56 scenes and passed open-diagonal production execution, but rejected recorded A* with `recording-source-identity-mismatch`. Investigation found 1,280 visible refined-concrete tiles had lost their hidden grass layer after prior scenes set their cells to out-of-map. Actors, visible geometry, mods and task metadata matched. The hub now restores the real grass substrate before shared static authoring, without excluding hidden tiles from hashes. All eleven rebuilt scenes match the original complete facts hashes, including after the full cross-domain selection sequence. A tight→open regression joins the integration suite. Original failure artifacts: local temp `scv-unified-lab-5qurytfd`; exact-facts recheck: `scv-unified-substrate-check-92t6c0pq`.

**Other constraints retained:** Gate surface deletion is queued until a native tick; paused reset must not falsely assert immediate deletion. The lab persists its gate sequence, evacuates players before release, and destroys old actors immediately. Native saved records are accepted only after current fact identity and shared admission; a recorded segment crossing the wall is rejected. Packaging validates all 44 source rows, not just the 33 imported ones, including native terminal/assertions and captured query identity. A no-player headless map cannot create LuaPlayer through the Factorio 2.0.77 API; GUI/connected-input coverage therefore stays explicitly unavailable rather than passing fake controls or launching a graphical simulation.

**Final native evidence:** Required `-Suite all -KeepArtifacts` passes on Factorio 2.0.77. Package `F:\factorio-scv-testbench\unified\v1-20260916-111550` retains the exact tested ZIP and isolated mods; `test-results.json` records all 56 selectable scenes, 89 identity-correlated fixed runs, 29 native lifecycle assertions and zero residual gate surfaces after subsequent native updates. No GUI player was fabricated: GUI coverage remains false. All 45 authoritative domain saves and 44 static comparisons also pass unchanged. Full suite artifact: `factorio-scv-agent-test-824933f45475405aa2dafc026f911869`; lab artifact: `scv-unified-lab-dbydt905`. Host discovery runs 115 tests with one optional dependency skip; pinned Python separately passes that dependency case. Independent review tightened menu-result correlation so an ignored algorithm selection or stale successful case cannot silently count as another passing matrix row.

**Decision:** Ship a single convenience lab alongside, not instead of, immutable source-map evaluation. Keep offline reference replay, production free play and domain fixed experiments visibly distinct. The [lab guide](unified-test-lab.md) documents operation, exact evidence and remaining manual GUI checks.
