...
| Solver comparison | Eleven persistent static source ZIPs now run with production-v1, captured-grid A*, Dijkstra and extremitypathfinder/GEOS source polygons. First complete replay passes all 44 rows, including four expected no-paths. Same-graph cost, shared native execution and timing scopes are separate checks; see [saved comparison](saved-solver-comparison.md). | Static stone-wall/tile domain only. Cold capture/admission remain expensive; dynamic update, memory, larger worlds and live production integration remain follow-up work. |
...
| Unified manual lab | One packaged save selects all 56 shared static/gate/dynamic/belt scenes, with fixed runs, reset and production free right-click. `-Suite unified` loads that same ZIP for 89 native fixed runs. | External menu entries replay validated fixed reference routes; they are not live arbitrary-goal solvers. Scene regeneration in this convenience lab does not replace immutable source ZIP evaluation. Native GUI/connected-input coverage is reported separately. See [lab guide](unified-test-lab.md). |
