...
- Python Dijkstra/A* solve the exported directed graph. Synthetic travel-time queries are supported offline; actual Factorio execution profiles accept distance only. The domain-calibration follow-up adds an optional pinned extremitypathfinder/GEOS source-polygon adapter and offline-only native replay; this is a bounded static-wall experiment, not dynamic navmesh, actual belt/gate support or production/live deployment.
- `/scv-nav-agent` and `tools/navigation/live.py` use short JSON RCON control/result messages and a default server-only bulk file for snapshots. The legacy chunked snapshot download remains a diagnostic comparison. File descriptors bind a fixed path, byte count, session/token, query hash and source hash. The envelope uses `operation`, session nonce, request token and sequential result-upload chunk index; it is not a general-purpose RPC implementation of every logical operation below. GUI use is an explicit manual connection to that lab, not an automated client launch.
